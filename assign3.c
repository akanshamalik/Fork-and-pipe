...
int main(int argc, char** argv) {
   ...
   static const char FILENAME[] = "large.txt";
    ...
    //integer pointer that will act as an array to hod every number in the text file.
...
//Integers variable that will be used to point to various loops.
    ...
    int status = 0;     // variable used to return the status of an exiting child process

    

    int userInput;     // variable use to store the user input for the number of process.(1, 2, or 4);

    

    int* arraySplit = malloc(sizeof(int));  // integer pointer that will act as the max number of times each process may read from its file.
 ...
 pipe(fd8_SEND);
    ...
    //long integers pinter that will store the offset to be sent to the children through the pipe.
...
//integers used to store the values from the test vile in each child process.
    ...
    unsigned long int* combinedFinalValue2 = malloc(sizeof(unsigned long int));
 ...
 //scan each line and store the value inside the data pointer
    ...
    // continue till it reach the end of the file. the formate specifier.
 ...
 clock_gettime(CLOCK_REALTIME, &begin);
    ...
    *arraySplit = *i/userInput;     // now the arraySplit value is  used for child process reading. Whatever i was
...

    {
        ...
        //that single child process will start to read form the very beginning, so its offset should be 0 bytes
  ...
  printf("offsetcalc2: %ld\n", *offsetCalc2);
        ...
        //The third offset value need to be 2 times the bytes of the offsetCacl2
        ...
        *offsetCalc3 = (*offsetCalc2 * 2);
        ...
        // the fourth offset value need to be 3 time the bytes of offsetcalc2.
...
fseek(fp, 0, SEEK_SET);     // set the file pointer back to the beginning
...

{
        ...
        // the parent will never enter this if statement; it will continue in

        //Creates a child process, store to id , and if the id == 0,(meaning its a child process) then enter the if condition is satisfied.
...

        {
            ...
            //the file pointer will move to the beginning of the file. R. offsetCalc1 is 0
 ...
 //the file pointer will move to the beginning of the file.
            ...
            while(fscanf(fp2, "%d", &valueGet2) == 1) {
                ...
                *processValue2 += valueGet2;                    // <---R to improve on
 ...
 printf("Child 1 sent value: %ld\n,", processValue1);
    ...
    read(fd8_SEND[0], offsetCalc4, sizeof(offsetCalc4));
     ...
     error("File Empty or non-exiting. Exiting system now");
           ...
           *processValue4 += valueGet4;
            ...
            // write the final process value to a new pipe that will send the value back to the parent
    ...
    if(userInput ==1)
        ...
        *combinedFinalValue2= *processValue1 + *processValue2 + *processValue3 + *processValue4;
